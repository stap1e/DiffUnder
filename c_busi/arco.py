...
def get_default_args(cli_dataset):
    if cli_dataset in DATASET_CONFIGS:
        ...
        defaults.setdefault('dataset', DEFAULT_BUSI_CFG['dataset'])
        return defaults
    return dict(DEFAULT_BUSI_CFG)


def get_parser(defaults):
    parser = argparse.ArgumentParser(description=defaults.get('dataset', DEFAULT_BUSI_CFG['dataset']))
    parser.add_argument('--dataset', type=str, default=defaults.get('dataset', DEFAULT_BUSI_CFG['dataset']))
    parser.add_argument('--base_dir', type=str, default=defaults.get('base_dir', DEFAULT_BUSI_CFG['base_dir']))
    parser.add_argument(
        '--labelnum',
        type=int,
        default=defaults.get('labelnum', DEFAULT_BUSI_CFG['labelnum']),
        help=defaults.get('label_help', DEFAULT_BUSI_CFG['label_help']),
    )
...
def compute_contra_memobank_loss(
    ...
    valid_count = 0
    for _, seg_proto, seg_feat_low_entropy in class_entries:
        bank = memobank[_][0]
...
if __name__ == '__main__':
    ...
    cp_path = os.path.join(
        ...
        'ARCO_BUSI/Ep{}bs{}_{}_seed{}_label{}/{}'.format(
            cfg['epochs'],
            cfg['batch_size'],
            cfg['dataset'],
            args.seed,
    ...
    include_list = ['c_busi', 'utils', 'configs', 'Datasets', 'models', 'scripts', 'tools']
