...
def abd_i(outputs1_max, outputs2_max, volume_batch, volume_batch_strong, label_batch, label_batch_strong, args, h_size, w_size, num_lb):
    image_patch_supervised_1 = rearrange(volume_batch[:num_lb].clone(), 'b c (h p1) (w p2) -> b (h w) c (p1 p2)', h=h_size, w=w_size, p1=args.patch_size, p2=args.patch_size)
    image_patch_supervised_2 = rearrange(volume_batch_strong[:num_lb].clone(), 'b c (h p1) (w p2) -> b (h w) c (p1 p2)', h=h_size, w=w_size, p1=args.patch_size, p2=args.patch_size)
    ...
    image_patch_supervised = rearrange(image_patch_supervised, 'b (h w) c (p1 p2) -> b c (h p1) (w p2)', h=h_size, w=w_size, p1=args.patch_size, p2=args.patch_size)
...
def abd_r(outputs1_max, outputs2_max, volume_batch, volume_batch_strong, outputs1_unlabel, outputs2_unlabel, args, h_size, w_size, num_lb, num_ulb):
    image_patch_1 = rearrange(volume_batch[num_lb:].clone(), 'b c (h p1) (w p2) -> b (h w) c (p1 p2)', h=h_size, w=w_size, p1=args.patch_size, p2=args.patch_size)
    image_patch_2 = rearrange(volume_batch_strong[num_lb:].clone(), 'b c (h p1) (w p2) -> b (h w) c (p1 p2)', h=h_size, w=w_size, p1=args.patch_size, p2=args.patch_size)
    ...
    return rearrange(image_patch, 'b (h w) c (p1 p2) -> b c (h p1) (w p2)', h=h_size, w=w_size, p1=args.patch_size, p2=args.patch_size)
...
def main(args, cfg, save_path, cp_path):
    ...
    model2 = init_2d_weight(UNet(in_chns=3, class_num=cfg['nclass']).cuda()).cuda()
...
if __name__ == '__main__':
    ...
    cp_path = os.path.join(
        ...
        'ABD_BUSI/Ep{}bs{}_{}_seed{}_label{}/cons{}_{}'.format(
