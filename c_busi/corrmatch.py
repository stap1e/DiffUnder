...
def main(args, cfg, save_path, cp_path):
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, (labeled_data, unlabeled_data) in enumerate(trainloader):
            ...
            img_u_w, img_u_s1, ignore_mask, cutmix_box1 = unlabeled_data
            ...
            mix_perm = torch.randperm(img_u_w.shape[0], device=img_u_w.device)
            img_u_w_mix = img_u_w[mix_perm]
            img_u_s1_mix = img_u_s1[mix_perm]
        ...
        if iter_num >= total_iters * args.val_start and epoch % args.val_interval == 0:
            model.eval()
            mDice, dice_class = eval_2d(valloader, model, cfg, ifdist=False, val_mode='CorrMatch_BUSI')
...
if __name__ == '__main__':
    ...
    thresh_tag = cfg.get('conf_thresh', args.thresh_init if args.thresh_init is not None else 0.95)
    cp_path = os.path.join(
        args.checkpoint_path,
        'CorrMatch_BUSI/Ep{}bs{}_{}_seed{}_label{}/thresh{}_{}'.format(
            cfg['epochs'], cfg['batch_size'], cfg['dataset'], args.seed, args.labelnum, thresh_tag, args.exp
        ),
    )
